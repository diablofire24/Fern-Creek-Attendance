...
class PrintSC():
        ...
        entry2 = tk.Entry(textvariable=var, width = 35, font=LARGE_FONT)
        entry2.pack()
        button3 = tk.Button(text="Enter", command=lambda: controller.code_print, width = 10, font=LARGE_FONT)
        button3.pack()
        button4 = tk.Button(text="Close", command=code_end, width = 10, font=LARGE_FONT)
        button4.pack()   
class FCHSapp():
        LARGE_FONT=("Verdana", 30)
        label = tk.Label(text="FCHS Teacher Login", font=LARGE_FONT)
        ...
        button1.pack()
        button2 = tk.Button(text="Print Absent/Tardy Teachers", command=lambda: controller.show_frame(PrintSC), width = 30, font=LARGE_FONT)
        button2.pack()
        def code_run(self):
                user = var.get()
                for i in codes.keys():
